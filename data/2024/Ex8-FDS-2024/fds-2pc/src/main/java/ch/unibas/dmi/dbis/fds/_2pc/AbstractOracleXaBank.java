...
public abstract class AbstractOracleXaBank {
    ...
    public abstract void transfer( AbstractOracleXaBank TO_BANK, String ibanFrom, String ibanTo, float value ) throws SQLException, XAException;
    ...
    public Xid startTransaction() throws XAException, SQLException {
        final Xid xid = this.getXid();
        this.getXaConnection().getConnection().setAutoCommit(false);
        // TODO: your turn ;-)
//        this.getXaConnection().getXAResource().start(xid,  XAResource.TMNOFLAGS);
        return xid;
//
...
//        throw new UnsupportedOperationException( "Implement me :-)" );
    ...
    public void endTransaction( final Xid transactionId, final boolean rollback ) throws XAException, SQLException {
        ...
        if(!rollback) {
            this.getXaConnection().getXAResource().end(transactionId, XAResource.TMSUCCESS);
        }
        else{
            this.getXaConnection().getXAResource().end(transactionId, XAResource.TMFAIL);
        }
//        throw new UnsupportedOperationException( "Implement me :-)" );
    }
